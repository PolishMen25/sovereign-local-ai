...
| CORE-700M | Architecture candidate et comptage exact vérifié | Configuration de 691 160 320 paramètres ; corpus anglais technique initial approuvé ; CORE-80M reste une référence historique | Aucune instanciation complète mesurée, aucun tokenizer final ni poids |
| Corpus / tokenizer | Contrat de promotion rejouable | Manifeste `0.2.0`, split `train` lié par taille/compte/SHA, Byte-BPE ordonné, NFC partagé, encode/decode et promotion `experimental` → `candidate_core` avec reçu | Qualité, couverture française et passage à l'échelle restent à traiter |
