...
def load_core_candidate_configuration(
    path: Path,
) -> tuple[dict[str, Any], CoreConfig, str]:
    """Read any candidate CORE configuration once, strictly and content-free.

    This is intentionally usable by the CORE-700M preflight without allocating
    its parameters.  Training still has its separate CORE-MINI-only gate.
    """
    try:
    ...
    if len(payload) != metadata_before.st_size or metadata_after.st_size != metadata_before.st_size:
        raise ValueError("Model configuration changed while being read")
    digest = hashlib.sha256(payload).hexdigest()
    try:
        document = json.loads(payload.decode("utf-8"), object_pairs_hook=_unique_object, parse_constant=_reject_json_constant)
    except (UnicodeDecodeError, json.JSONDecodeError, ValueError):
        raise ValueError("Model configuration is not strict JSON") from None
    finally:
        payload = b""
    if not isinstance(document, dict) or document.get("status") != "candidate":
        raise ValueError("Expected a candidate model configuration")
    ...
    if not strict_recursive_equal(document.get("parameter_count"), count_parameters(config)):
...
def load_core_mini_configuration(
    ...
    document, config, config_sha256 = load_core_candidate_configuration(path)
